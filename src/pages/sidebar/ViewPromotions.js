...
const ViewPromotions = () => {
    ...
    return (
        <div className="view-promotions">
            ...
            <div className="vpromo-container">
                <div className="vpromo-filter">
                    {categories.map((category) => (
                        <button
                            key={category}
                            className={`vfilter-btn ${selectedCategory === category ? 'active' : ''}`}
                ...
                <div className="vpromo-grid">
                    {filteredPromos.map((promo) => (
                        <div 
                            key={promo.id}
                            className="vpromo-card"
                            onClick={() => setActivePromo(activePromo === promo.id ? null : promo.id)}
                        >
                            <div className="vpromo-top">
                                <h2>{promo.title}</h2>
                                <div className="vdiscount-badge">{promo.discount}</div>
                            </div>
                            ...
                            <div className="vpromo-footer">
                                ...
                                <span className="vcategory-tag">{promo.category}</span>
                            </div>

                            {activePromo === promo.id && (
                                <div className="vpromo-expanded">
                                    <div className="vcode-display">
                                        <span className="vcode-label">Promo Code:</span>
